...
const ENHANCEMENT_OPTIONS = [
  {
    id: "Default",
    label: "Default",
    subtitle: "Run the normal retrieval pipeline.",
  },
  {
    id: "Query Rewrite",
    label: "Query Rewrite",
    subtitle: "Rewrite the user's question.",
  },
  {
    id: "HyDE",
    label: "HyDE",
    subtitle: "Generate hypothetical answer embeddings.",
  },
  {
    id: "Multi Query",
    label: "Multi Query",
  ...
  },
  {
    ...
    subtitle: "Expand the query.",
  },
  {
    id: "All",
    label: "All",
    subtitle: "Enable every enhancement.",
  },
];
...
// ── Component ──────────────────────────────────────────────────────────────
// onRunChange: optional callback — called with the active run object whenever
//              it changes, so GaugePilot can pass it down to AIAnalysis.

// Safe JSON parse — returns null instead of throwing on bad/missing input.
const safeParseLeaderboard = (json) => {
...
export default function ExperimentSetup({ onRunChange }) {
  ...
  // ── Bubble selectedRun up to parent ───────────────────────────────────────
  useEffect(() => {
    onRunChange?.(selectedRun);
  }, [selectedRun]);  // eslint-disable-line react-hooks/exhaustive-deps

  // ── Close enhancement dropdown on outside click ────────────────────────────
  useEffect(() => {
    const handleClick = (e) => {
      if (enhancementRef.current && !enhancementRef.current.contains(e.target)) {
    ...
    const loadDashboardData = async () => {
      try {
        ...
        if (sorted.length) {
          ...
          const leaderboard = safeParseLeaderboard(sorted[0].leaderboard_json);
  ...
  const handleRun = async () => {
    ...
    const payload = {
      model,
      retrieval_method: retrievalMap[retrievalMethod],
      reranker,
    ...
    try {
      ...
      if (sorted.length) {
        ...
        setSelectedLeaderboard(
          res?.leaderboard ?? safeParseLeaderboard(sorted[0].leaderboard_json)
        );
      }
    } catch (err) {
  ...
  const handleSelectRun = (runId) => {
    ...
    setSelectedRun(run);
      ...
      const parsed = safeParseLeaderboard(run.leaderboard_json);
      if (parsed) {
        setSelectedLeaderboard(parsed);
      } else {
  ...
  const handleDeleteRun = async () => {
    ...
    try {
      ...
      if (sorted.length) {
        setSelectedRun(sorted[0]);
        setSelectedLeaderboard(safeParseLeaderboard(sorted[0].leaderboard_json));
      } else {
        setSelectedRun(null);
        setSelectedLeaderboard(null);
        setBestScore(null);
    ...
    >
      ...
      <div style={{ ...card, marginBottom: "16px" }}>
        ...
        <div style={{
          ...
          borderTop: allRuns.length > 0 ? "1px solid rgba(255,255,255,0.08)" : "none",
        }}>
      ...
      <div style={{ ...card, marginBottom: "16px" }}>
        ...
        <div style={{ display: "flex", gap: "32px", flexWrap: "wrap" }}>
          <ExperimentSelector
            ...
            onChange={setModel}
            options={[
              { value: "llama-3.1-8b",  label: "Llama 3.1 8B",  description: "Fast & Efficient"          },
              { value: "llama-3.3-70b", label: "Llama 3.3 70B", description: "Best Overall"              },
              { value: "llama-4-scout", label: "Llama 4 Scout", description: "Large Context Window"      },
              { value: "qwen-3-32b",    label: "Qwen 3 32B",    description: "Reasoning & Analysis"      },
              { value: "gpt-oss-20b",   label: "GPT OSS 20B",   description: "Fast Reasoning"            },
              { value: "gpt-oss-120b",  label: "GPT OSS 120B",  description: "Deep Reasoning"            },
            ]}
          />
          <ExperimentSelector
            ...
            options={[
              { value: "FAISS",  label: "FAISS",  description: "Dense vector retrieval"   },
              { value: "BM25",   label: "BM25",   description: "Sparse keyword retrieval" },
              { value: "Hybrid", label: "Hybrid", description: "Dense + sparse retrieval" },
            ]}
          />
          <ExperimentSelector
            label="Reranker"
            ...
            options={[
              { value: "none",      label: "None",      description: "No reranking"                          },
              { value: "minilm",    label: "MiniLM",    description: "Fast balanced baseline"                },
              { value: "tinybert",  label: "TinyBERT",  description: "Ultra-fast lightweight"                },
              { value: "bge-large", label: "BGE Large", description: "High-quality semantic ranking"         },
              { value: "bge-m3",    label: "BGE M3",    description: "Modern retrieval-focused reranker"     },
            ]}
          />
          <CustomSelector
          ...
          >
            {ENHANCEMENT_OPTIONS.map((opt) => (
              <SelectorItem
                ...
                active={
                  opt.id === "All"
                    ? ALL_ENHANCEMENT_IDS.every((e) => selectedEnhancements.includes(e))
      ...
      <div style={{
            ...
            {uploading ? (
              <>
                <div style={{
                  ...
                  fontSize: "28px", boxShadow: "0 0 20px rgba(79,110,247,0.25)",
                }}>
                  ⏳
                </div>
                <div style={{ textAlign: "center", zIndex: 1 }}>
                  <p style={{ margin: 0, fontWeight: 700, fontSize: "15px", color: "#7b96ff" }}>
                    Uploading & Indexing...
                  </p>
              ...
              <>
                <div style={{
                  ...
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: "28px", boxShadow: "0 0 20px rgba(34,197,94,0.25)",
                }}>
                  ✅
                </div>
                <div style={{ textAlign: "center", zIndex: 1 }}>
                  <p style={{ margin: 0, fontWeight: 700, fontSize: "15px", color: green }}>
                    {uploadedFile.name}
                  </p>
                  <p style={{ margin: "4px 0 0", fontSize: "12px", color: "rgba(255,255,255,0.5)" }}>
              ...
              <>
                <div style={{
                  ...
                  background: isDragging ? `rgba(79,110,247,0.25)` : "rgba(79,110,247,0.14)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: "28px", zIndex: 1,
                  boxShadow: isDragging ? `0 0 24px rgba(79,110,247,0.35)` : "none",
                  transition: "all 0.2s ease",
                }}>
                  ⬆️
                </div>
                <div style={{ textAlign: "center", zIndex: 1 }}>
                  ...
                  <p style={{ margin: "5px 0 0", fontSize: "12px", color: "rgba(255,255,255,0.5)", lineHeight: 1.6 }}>
          ...
          <div style={{ ...card, padding: "16px 20px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0px" }}>
              {[
                ...
                { label: "Enhancements", value: selectedEnhancements.join(", ") },
                ...
                >
                  ...
                  <span style={{ fontSize: "12px", fontWeight: 600, color: "rgba(255,255,255,0.9)" }}>{value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ ...card, display: "flex", flexDirection: "column", gap: "14px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <p style={{ ...sectionLabel, marginBottom: "3px", color: "rgba(255,255,255,0.85)" }}>
      ...
      <div id="visualizations" style={{ marginTop: "24px" }}>
